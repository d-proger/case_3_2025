...
// Получаем список user_id, на которых подписан текущий пользователь
$stmt = $pdo->prepare("SELECT author_id FROM subscribes WHERE user_id = :user_id");
$stmt->execute([':user_id' => $user_id]);
$user_ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

// Если пользователь ни на кого не подписан, выводим сообщение
if (empty($user_ids)) {
    echo "<div class='container mt-4'><div class='alert alert-info'>Вы пока ни на кого не подписаны.</div></div>";
...
$sql = "
    ...
    WHERE p.user_id IN ($user_ids_string)
";
